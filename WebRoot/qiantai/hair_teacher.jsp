	...
	<script type="text/javascript">
		function showQc(id){
          var d = top.dialog({
                width:600,
                height:330,
                url:'user.do?method=showQc&id='+id,//可以是一个访问路径Action|Servlet等或者jsp页面资源
                onclose: function () {
		...
		function chuzuQc(id){
          var d = top.dialog({
                ...
                width:300,
                height:250,
                url:'web.do?method=chuzuQc&id='+id,//可以是一个访问路径Action|Servlet等或者jsp页面资源
                onclose: function () {
                if (this.returnValue="success") {
                    window.location.href= window.location.href;
                }
...
<div id="container" class="container clearfix">
    ...
    <div class="left">
		...
		%>
    <!--right-->
    <form action="user.do?method=selQc" method="post" rel="page">
    <div class="right">
    	<div class="filterbox">	
        ...
        <c:if test="${!empty list}">
			<c:forEach items="${list}" var="list" varStatus="vs">
                    <li class="clearfix">
                    	<div class="face"><a href="javascript:void(0);" onclick="showQc('${list.id}');"><img src="<%=basePath %>image/${list.photo}" width="100%" height="100%"  alt=""/></a></div>
                        <div class="text">
                        	<a href="#">姓名：${list.hairTeacherName} </a>
                        	<p class="zc">工作年限：${list.level}年</p>
                        	<p class="zc">联系电话：${list.phone}</p><%--
                            <p class="jj"><a href="javascript:void(0);" onclick="chuzuQc('${list.id}');"><font color="red">预约</font></a></p>
                        --%></div>
                  	</li>
                  		</c:forEach>
		</c:if>
		<c:if test="${empty list}">
			<tr>
				<td colspan="8" align="center">
					<strong><font color="red" size="6">暂时没有教练信息，请重新输入!!!</font></strong>
				</td>
			</tr>
		</c:if>
        </ul>
